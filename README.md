...
## エラー
* 提出データの勘違い。提出すべきはtestデータに対する予測結果。
* GPUの過負荷。同一のモデルを複数箇所で読み込んでいた。初期化は1回で。
...
## 反省会
### 良かった点
* 短期間でKaggleの流れを知ることができた
* Baseline +αとして画像分割と画像回転を実装できた
* 最終的に順位を上げた

### 改善点
1. 習慣化する（Kaggle日記） データを見て試行錯誤を増やす。（今回試せた手法は1つだけ）。画像分割と回転もコード例が掲示板にあった...
2. 掲示板やSNSのチェック。目的はトレンド手法のキャッチアップ(画像→3D再構成のトレンド手法：VGGT)
3. アプローチやモデルの理解のために、基本的な部分は自分で１から実装してみる。（コード生成を用いるのではなく）
